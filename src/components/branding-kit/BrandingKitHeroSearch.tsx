...
import { Building2, Loader2, Palette, Search } from "lucide-react";
...
export default function BrandingKitHeroSearch({
  onSearch,
  onSelectBusiness,
  searching,
  variant = "standalone",
  ...
  const sectionClass =
    variant === "homepage"
      ? "hero-sky relative overflow-hidden px-4 pb-16 pt-24 sm:px-6 sm:pt-28 lg:px-8 lg:pt-32"
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-text-secondary sm:text-xl">
            Search your Google Business Profile and we&apos;ll assemble a downloadable brand kit plus sales funnel
            playbook—positioning, creative direction, and funnel stages tailored to your listing.
          </p>
        </div>
        ...
        >
          <label className="relative block">
            ...
            {open && (suggestions.length > 0 || suggesting || hint) ? (
              <div className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-50 overflow-hidden rounded-2xl border border-border bg-white text-left shadow-soft-lg dark:border-zinc-700 dark:bg-zinc-900">
                {suggesting ? (
                ...
                {suggestions.map((result) => (
                  <button
                    ...
                    className="block w-full border-t border-border/60 px-4 py-3 text-left transition-colors first:border-t-0 hover:bg-surface dark:hover:bg-zinc-800/70"
                  >
                    <span className="block text-sm font-bold text-text-primary">{result.name}</span>
                    <span className="mt-1 block text-xs text-text-secondary">
                      {result.address ?? "Address unavailable"} · {result.rating ?? "N/A"} rating ·{" "}
        ...
        {variant !== "homepage" ? (
          <p className="mt-4 text-center text-sm text-text-secondary">
            Pick your listing from suggestions for the fastest, most accurate kit.
          </p>
        ) : null}
      </div>
    </section>
