...
export default function ComingSoonModule({
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-6 text-center sm:py-10">
      ...
      <h1 className="heading-display text-xl font-bold text-text-primary dark:text-zinc-100 sm:text-2xl">
        {title}
      </h1>
      ...
      <ul className="mt-8 space-y-3 text-left">
        {features.map((text) => (
          <li
            key={text}
            className="flex gap-3 rounded-2xl border border-border bg-white px-4 py-3 text-sm text-text-primary dark:border-zinc-800/80 dark:bg-zinc-900/40 dark:text-zinc-200"
          >
