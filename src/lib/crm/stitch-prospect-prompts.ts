...
1. **#dash** — The **hero view** that sells the product:
   ...
   - **KPI band:** One **hero metric card** (2x width of others, 48–64px display number, trend arrow + percentage, accent background or gradient) flanked by 2–3 supporting stat cards (today’s bookings, revenue, open tasks) each with their own trend indicator.
   - **CSS chart:** A visually impressive **bar chart or visualization** using CSS-only (flexbox bars, grid, or styled divs) with realistic weekly/daily data, axis labels, and a total. Must look complete and polished — not a placeholder rectangle.
   - **Needs attention:** Exception list with 3–4 items, each with a **colored left-border** (4px) for severity (red = urgent, amber = warning, green = ready), avatar/icon, client name, service, and status badge as a colored pill.
...
1. **#home** (dashboard) — The **hero screen** that sells the product:
   ...
   - **KPI hero:** One oversized metric (40px+ display font) for today’s primary number (bookings, revenue, or appointments) with trend indicator (arrow + percentage). Flanked by 1–2 supporting stats in smaller cards or pills.
   - **Today’s schedule:** 3+ appointment cards, each with: circular avatar placeholder (colored initial), client name, service type, time, and **colored status badge** (pill format: “Confirmed” in green, “Pending” in amber, etc.). Cards must have shadow, rounded corners, and a service-type color accent (left border or icon).
   ...
   - **New request indicator:** A notification-style card or badge for pending requests with chevron.

2. **#clients** — Search bar with styled input (rounded, icon); **vertical list** with: **circular avatar** (colored initial), client name, service history or last visit, status badge. Minimum 4 realistic rows with subtle card separation (shadow or divider). **“Add Client”** floating action button (accent color, circular, bottom-right corner with shadow). Each row: chevron, 48px+ tap target height.

3. **#book** — **Premium booking experience:**
   ...
   - **Booking summary card:** Below slots — selected service, date, time, with a prominent “Confirm Booking” gradient/accent button. Must feel like a **real booking system**.

4. **#reviews** — Large aggregate rating (48px number + filled star icons + review count). Visual star distribution bar (5-star to 1-star with horizontal bars). 2+ review cards: circular author avatar (initial), star rating (filled/empty), realistic review text specific to this business type, date. **“Request a Review”** CTA card at the bottom with accent styling and an icon.
